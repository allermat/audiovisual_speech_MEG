% Maxfilter 2.2 Matlab script (runs on Linux)
% 
% Heidi Solberg Okland, April 2017
%
% Adapted from Olaf's Maxfilter_main_script_OH.m
% and Lucy's ambiguity_maxfilter_main_script_160106.m
%
% Dependent on the functions 'maxfilter_one_subject.m' and
% 'maxfilter_get_bad_channels_from_fiff.m'
%
% Note: takes about 30-40 minutes per ~12 minutes (= 1 run) of MEG data
% when you run it on a login machine
%
% Olaf's instructions:
%
...
ParType = 2;   % parallelisation options 

% overwrite exisiting fif-files?
...
% then specify do_eval = 0, else set to 1
datinfo.do_eval = 1;
...
% general log-file, records which fif-files have been successfully created
...
% BEFORE YOU RUN:
% 1) Specify subject directories
% 2) Specify bad channels

% Raw data will be read from subdirectories of...
datinfo.data_in_dir = '/megdata/cbu/av_speech/'; 

% Results will be written to subdirectories of...
datinfo.data_out_dir = '/imaging/hs01/AV_speech/Maxfiltered/';
...
datinfo.raw_files_in = {'block1_raw', 'block2_raw', 'block3_raw', 'block4_raw', 'block5_raw'};

% output names of raw data files - if only one list provided, it will be used for all subjects
datinfo.raw_files_out = {'block1_raw', 'block2_raw', 'block3_raw', 'block4_raw', 'block5_raw'};
...
cstr.maxfstr = '!/hpc-software/neuromag/bin/util/maxfilter-2.2.12 '; % this specifies the Maxfilter version
...
%         now_file  = fullfile(datinfo.data_in_dir, datinfo.sub_dirs{ss}, [datinfo.raw_files_in{ss}{rr} '.fif']);        
         now_file  = fullfile(datinfo.data_in_dir, datinfo.sub_dirs{ss}, [datinfo.raw_files_in{ss} '.fif']);        
